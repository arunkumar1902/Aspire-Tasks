...
-- session variable @variable_name --> declaration outside procedure
select * from employee;
set @salaryUpdate =0;

delimiter $$
create procedure salaryUpdate (in id int, out updatedSalary numeric) 
begin
declare currentSalary numeric;
select salary into currentSalary from employee where employee_id = id;
set updatedSalary = currentSalary + 5000;
update employee set salary = updatedSalary where employee_id = id;
select * from employee;
end $$
delimiter;

call salaryUpdate(101,@updatedSalary);
